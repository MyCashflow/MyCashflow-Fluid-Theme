...
(function($) {
	...
	var VariationsSplitter = {
		...
		render: function(formData, state) {
			...
			var firstMatch = formData.variations.find(function(variation) {
				if (state.length > 0) {
					return variation.path.join('.').startsWith(state.join('.'));
				}
			});
			...
			this.match(firstMatch, selected);
			return $items;
		},
	...
	$.extend(true, window, { MCF: { VariationsSplitter: VariationsSplitter } });
})(jQuery);
